...
void painter::painteRealLine(QJsonValue jsonvalue, QWidget *widget)
{
    ...
    //获取数据
    QJsonArray arrayFirst = jsonvalue.toObject().value(data).toArray();
    QJsonArray array;
    double _yestclose = 0.0;
    if (arrayFirst.size() > 0)
    {
        _yestclose = arrayFirst[0].toObject().value(yest_close).toString().toDouble();
    }
    //绘制分时线图数据
    ...
    n_painter.setPen(n_pen);
    if (!name.isEmpty())
        n_painter.drawText(width - 120, 10, QString("股 票 名:") + name);
    if (!code.isEmpty())
        n_painter.drawText(width - 120, 25, QString("股票代码:") + code);
    //绘制刻度
    //刻度画笔
    QPainter yTextPainter(widget);
    QPen yTextPen;
